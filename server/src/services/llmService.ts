...
export class LlmService {
  ...
  public async extractEntities(
    ...
    if (this.aiClient) {
      try {
        const entities = await this.extractWithGemini(prompt);
    ...
    const entities = this.heuristicExtractEntities(prompt);
   ...
   */
  private async extractWithGemini(prompt: string): Promise<ExtractedEntities> {
    if (!this.aiClient) {
              ...
              items: {
                ...
                properties: {
                  ...
                  explicitUnitPrice: {
                    type: SchemaType.NUMBER,
                    description:
                      "Unit price per item if stated, otherwise omit.",
                  },
                  explicitTaxRate: {
                    type: SchemaType.NUMBER,
                    description:
   ...
   */
  public heuristicExtractEntities(prompt: string): ExtractedEntities {
